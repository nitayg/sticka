...
>(({ className, children, ...props }, ref) => {
  // Get the RTL setting from the dir attribute on the element or its parent
  const contentRef = React.useRef<HTMLDivElement>(null);
  const [isRtl, setIsRtl] = React.useState(false);
  
  React.useEffect(() => {
    if (contentRef.current) {
      const dir = contentRef.current.getAttribute('dir') || 
                  document.dir || 
                  getComputedStyle(contentRef.current).direction;
      setIsRtl(dir === 'rtl');
    }
  }, []);
